...
class HashTable:
    ...
    def delete_by_key(self, key):
        ...
        """
        ======================== 중요 =====================
        주어진 key에 해당하는 노드를 찾음
        HDLL의 delete는 링크드리스트의 메서드이다. 노드의 메서드가 아니다.
        node_to_delete 로 삭제하는 노드도 인수로 전달해야하기 때문에 노드, 링크드리스트 객체 둘다 필요하다
        ==================================================
        """
        node_to_del = self._look_up_node(key)
        table_linked_list = self._get_linked_list_for_key(key)
        
        if node_to_del is None:
            return None
        table_linked_list.delete(node_to_del)
