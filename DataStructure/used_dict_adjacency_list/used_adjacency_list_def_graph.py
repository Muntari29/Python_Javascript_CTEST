    ...
    with open(input_file) as stations_raw_file:
        for line in stations_raw_file:  # 파일을 한 줄씩 받아온다
            ...
            for name in subway_line:
                ...
                # 첫번째 역 일때는 previous_station 는 None
                # 첫번째 역을 제외하고 현재 역의 인접리스트에 이전 역의 노드를 저장해줌 
                if previous_station is not None:
                    current_station.add_connection(previous_station)
                
                # 반복문을 돌며 previous_station이 계속해서 갱신시킴.
                previous_station = current_station
            
    return stations


stations = create_subway_graph("/Users/munseunghui/playground/codingtest/Python_Javascript_CTEST/DataStructure/used_dict_adjacency_list/test_stations.txt")  # stations.txt 파일로 그래프를 만든다
